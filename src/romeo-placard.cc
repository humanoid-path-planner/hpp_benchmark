...
std::map<std::string, double>
    leftHandClosed = {{"LHand", 0.0},     {"LFinger12", 0.0},
                      {"LFinger13", 0.0}, {"LFinger21", 0.0},
                      {"LFinger22", 0.0}, {"LFinger23", 0.0},
                      {"LFinger31", 0.0}, {"LFinger32", 0.0},
                      {"LFinger33", 0.0}, {"LThumb1", 0.0},
                      {"LThumb2", 0.0},   {"LThumb3", 0.0}},
    leftHandOpen = {{"LHand", 1},        {"LFinger12", 1.06},
                    {"LFinger13", 1.06}, {"LFinger21", 1.06},
                    {"LFinger22", 1.06}, {"LFinger23", 1.06},
                    {"LFinger31", 1.06}, {"LFinger32", 1.06},
                    {"LFinger33", 1.06}, {"LThumb1", 0},
                    {"LThumb2", 1.06},   {"LThumb3", 1.06}},
    rightHandClosed = {{"RHand", 0.0},     {"RFinger12", 0.0},
                       {"RFinger13", 0.0}, {"RFinger21", 0.0},
                       {"RFinger22", 0.0}, {"RFinger23", 0.0},
                       {"RFinger31", 0.0}, {"RFinger32", 0.0},
                       {"RFinger33", 0.0}, {"RThumb1", 0.0},
                       {"RThumb2", 0.0},   {"RThumb3", 0.0}},
    rightHandOpen = {
        {"RHand", 1},        {"RFinger12", 1.06}, {"RFinger13", 1.06},
        {"RFinger21", 1.06}, {"RFinger22", 1.06}, {"RFinger23", 1.06},
        {"RFinger31", 1.06}, {"RFinger32", 1.06}, {"RFinger33", 1.06},
        {"RThumb1", 0},      {"RThumb2", 1.06},   {"RThumb3", 1.06}};

namespace hpp {
namespace benchmark {

std::vector<std::string> createLockedJointForHandsOpen(
    ProblemSolverPtr_t ps, const std::string& prefix) {
  ...
  for (const auto& dict : {rightHandOpen, leftHandOpen}) {
    for (const auto& pair : dict) {
      ...
      ps->addNumericalConstraint(
          n, constraints::LockedJoint::create(
                 joint, joint->configurationSpace()->element(q)));
    }
  }
  return names;
}

std::vector<std::string> createConstraints(ProblemSolverPtr_t ps,
                                           Configuration_t q0,
  ...
  ps->addNumericalConstraint(
      name, constraints::Implicit::create(
                constraints::Transformation::create(
                    name, robot, ra, Transform3f::Identity(), oMr),
                6 * EqualToZero));
...
class implicit_versus_explicit : public BenchmarkNCase {
 private:
  ...
  std::map<intint_t, std::string> implicitGraspConstraints,
      explicitGraspConstraints;
 ...
 public:
  ...
  void setup(int NIter) {
    ...
    pinocchio::urdf::loadModel(
        device, 0, "romeo", "freeflyer",
        "package://example-robot-data/robots/romeo_description/urdf/romeo.urdf",
        "package://example-robot-data/robots/romeo_description/srdf/"
        "romeo.srdf");
    srdf::loadModelFromFile(device, "romeo",
                            "package://example-robot-data/robots/"
                            "romeo_description/srdf/romeo.srdf");

    device->controlComputation(
        (pinocchio::Computation_t)(pinocchio::JOINT_POSITION |
                                   pinocchio::JACOBIAN));
    ps->robot(device);
    ...
    std::vector<std::string> lockHands(
        createLockedJointForHandsOpen(ps, "romeo/"));
    std::vector<std::string> balance(
        createConstraints(ps, q0, "romeo/LAnkleRoll", "romeo/RAnkleRoll"));
        ...
        ps->addNumericalConstraint(
            name, constraints::Implicit::create(
                      constraints::RelativeTransformation::create(
                          name, device, _gripper->joint(), _handle->joint(),
                          _gripper->objectPositionInJoint(),
                          _handle->localPosition()),
                      6 * EqualToZero));
  ...
  void initializeProblem(int iIter, int iCase) {
    ...
    constraint =
        core::ConfigProjector::create(ps->robot(), "constraint", 1e-4, 40);
    currentCaseIsExplicit = false;
    switch (iCase) {
      case 0:
        constraint->add(ps->numericalConstraints.get(
                            implicitGraspConstraints[intint_t(1, 1)]),
                        0);
        break;
      case 1:
        constraint->add(ps->numericalConstraints.get(
                            implicitGraspConstraints[intint_t(0, 0)]),
                        0);
        break;
      case 2:
        constraint->add(ps->numericalConstraints.get(
      ...
      case 3:
        constraint->add(ps->numericalConstraints.get(
                            explicitGraspConstraints[intint_t(1, 1)]),
                        0);
        currentCaseIsExplicit = true;
        break;
      case 4:
        constraint->add(ps->numericalConstraints.get(
                            explicitGraspConstraints[intint_t(0, 0)]),
                        0);
        currentCaseIsExplicit = true;
        break;
      case 5:
